...
export async function fetchKlines({ interval, limit, symbol }) {
  ...
  if (!res.ok) {
    throw new Error(`Binance klines error (${symbol}): ${res.status} ${await res.text()}`);
  }
...
export async function fetchLastPrice(symbol) {
  ...
  const res = await fetch(url);
  if (!res.ok) {
    throw new Error(`Binance last price error (${symbol}): ${res.status} ${await res.text()}`);
  }
  const data = await res.json();
